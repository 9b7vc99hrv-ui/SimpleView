...
namespace BasicDemo
{
    public partial class Form1 : Form
    {
        ...
        private int DisplayImage(MV3D_LP_IMAGE_DATA pstImage)
        {
            ...
            //// 解决方案2 | Solution 2
            //byte[] ImageBuffer = new byte[(int)pstImage.nDataLen];
            //Marshal.Copy(pstImage.pData, ImageBuffer, 0, (int)pstImage.nDataLen);
            //var handle = GCHandle.Alloc(ImageBuffer, GCHandleType.Pinned);
            //IntPtr pImage = handle.AddrOfPinnedObject();
            //Bitmap bmp = new Bitmap((int)pstImage.nWidth, (int)pstImage.nHeight, (int)pstImage.nWidth, PixelFormat.Format8bppIndexed, pImage);
        ...
        public void ReceiveThreadProcess()
        {
            ...
            MV3D_LP_IMAGE_DATA stIntensityImageData = new MV3D_LP_IMAGE_DATA();
            while (this.m_bGrabbing)
            {
                ...
                if (nRet == (int)Mv3dLpSDK.MV3D_LP_OK)
                {
                    try
                    {
                        if(0 >= stImageData.nIntensityDataLen)
                        {
                            continue;
                        }

                        // 渲染亮度图 | Display Intensity
                        stIntensityImageData.enImageType = Mv3dLpSDK.ImageType_Mono8;
                        stIntensityImageData.nWidth      = stImageData.nWidth;
                        stIntensityImageData.nHeight     = stImageData.nHeight;
                        stIntensityImageData.pData       = stImageData.pIntensityData;
                        stIntensityImageData.nDataLen    = stImageData.nIntensityDataLen;
                        stIntensityImageData.nFrameNum   = stImageData.nFrameNum;
                        nRet = DisplayImage(stIntensityImageData);
                        if ((int)Mv3dLpSDK.MV3D_LP_OK != nRet)
                        {
