...
namespace BasicDemo
{
    public partial class Form1 : Form
    {
        ...
        static byte[] m_pcRangeBuf = new byte[m_MaxImageSize];
        static byte[] m_pcRangeBuf1 = new byte[m_MaxImageSize];
        static byte[] m_pcRangeBuf2 = new byte[m_MaxImageSize];
        static byte[] m_pcIntensityBuf = new byte[m_MaxImageSize];
        static byte[] m_pcIntensityBuf1 = new byte[m_MaxImageSize];
        static byte[] m_pcIntensityBuf2 = new byte[m_MaxImageSize];
        ...
        private void bnOpen_Click(object sender, EventArgs e)
        {
            ...
            bnGetParam_Click(null, null);// 获取参数 | Get parameters
        ...
        public void ReceiveThreadProcess()
        {
            ...
            stImageDataVector[0].pData = Marshal.UnsafeAddrOfPinnedArrayElement(m_pcRangeBuf, 0);
            stImageDataVector[0].pIntensityData = Marshal.UnsafeAddrOfPinnedArrayElement(m_pcIntensityBuf, 0);

            stImageDataVector[1].pData = Marshal.UnsafeAddrOfPinnedArrayElement(m_pcRangeBuf1, 0);
            stImageDataVector[1].pIntensityData = Marshal.UnsafeAddrOfPinnedArrayElement(m_pcIntensityBuf1, 0);

            stImageDataVector[2].pData = Marshal.UnsafeAddrOfPinnedArrayElement(m_pcRangeBuf2, 0);
            stImageDataVector[2].pIntensityData = Marshal.UnsafeAddrOfPinnedArrayElement(m_pcIntensityBuf2, 0);
            ...
            while (this.m_bGrabbing)
            {
                ...
                if (nRet == (int)Mv3dLpSDK.MV3D_LP_OK)
                {
                    ...
                    {
                        // 测试代码 | Test Code
                        stImageDataVector[nCount].nWidth = stImageData.nWidth;
                        stImageDataVector[nCount].nHeight = stImageData.nHeight;
                        stImageDataVector[nCount].nDataLen = stImageData.nDataLen;
                        stImageDataVector[nCount].enImageType = stImageData.enImageType;
                        stImageDataVector[nCount].nFrameNum = stImageData.nFrameNum;
                        stImageDataVector[nCount].fXScale = stImageData.fXScale;
                        stImageDataVector[nCount].fYScale = stImageData.fYScale;
                        stImageDataVector[nCount].fZScale = stImageData.fZScale;
                        stImageDataVector[nCount].nXOffset = stImageData.nXOffset;
                        stImageDataVector[nCount].nYOffset = stImageData.nYOffset;
                        stImageDataVector[nCount].nZOffset = stImageData.nZOffset;

                        if (nCount == 0)
                        {
                            Marshal.Copy(stImageData.pData, m_pcRangeBuf, 0, (int)stImageData.nDataLen);
                            Marshal.Copy(stImageData.pIntensityData, m_pcIntensityBuf, 0, (int)stImageData.nIntensityDataLen);
                        }

                        if (nCount == 1)
                        {
                            Marshal.Copy(stImageData.pData, m_pcRangeBuf1, 0, (int)stImageData.nDataLen);
                            Marshal.Copy(stImageData.pIntensityData, m_pcIntensityBuf1, 0, (int)stImageData.nIntensityDataLen);
                        }

                        if (nCount == 2)
                        {
                            Marshal.Copy(stImageData.pData, m_pcRangeBuf2, 0, (int)stImageData.nDataLen);
                            Marshal.Copy(stImageData.pIntensityData, m_pcIntensityBuf2, 0, (int)stImageData.nIntensityDataLen);
                        }
